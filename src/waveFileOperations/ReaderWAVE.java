...
public class ReaderWAVE {
    ...
    public void detecionOfWord(File filesToRead) throws WavFileException, IOException, NullPointerException {

    }

    public void setFramesFromRecordsSamples(File fileToRead) throws IOException, WavFileException, NullPointerException {
        try {
            WavFile file = WavFile.openWavFile(fileToRead);
            int noOfChannel = file.getNumChannels();
            long noFrames = file.getNumFrames();
            this.sampleRate = (int)file.getSampleRate();
            int sizeOfBufferToRead = (int)noFrames * noOfChannel;
            double [] buffer = new double[sizeOfBufferToRead];

            file.readFrames(buffer, sizeOfBufferToRead);
